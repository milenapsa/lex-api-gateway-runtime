...
def _memory_status(handler: gateway.H, parsed: urllib.parse.ParseResult) -> None:
    if not MEMORY_UPSTREAM: 
        ...
        return
    try:
        status, body, _ = gateway.legacy.call_json_upstream(
            MEMORY_UPSTREAM, "/v1/memory/status", "GET"
        )
        ...
        try:
            ...
            data = {"error": "memory_upstream_http_error", "status": error.code}
        handler.sendj(error.code, data)
    except (urllib.error.URLError, TimeoutError) as error:
        handler.sendj(502, {"error": "memory_upstream_unavailable", "detail": str(error)[:160]})
    except Exception as error:
        handler.sendj(500, {"error": "gateway_error", "detail": error.__class__.__name__})
...
def main() -> None:
    ThreadingHTTPServer(("0.0.0.0", gateway.legacy.PORT), gateway.H).serve_forever()
